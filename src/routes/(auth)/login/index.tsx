...
export const useLoginUserAction = routeAction$((data, { cookie, redirect }) => {
    ...
    if (email == 'jhon@google.com' && password == '123456') {
        cookie.set('jwt', 'este_es_un_jwt', { secure: true, path: '/' })
        redirect(302, '/')
        return {
            success: true,
            jwt: 'este_es_un_jwt'
    ...
    return (
        <Form action={ action } class="login-form mt-5" >
            ...
            <code style="height: 200px;">
                <p> { JSON.stringify( action.status, undefined , 2 ) } </p>
                <p> { JSON.stringify( action.value, undefined , 2 ) } </p>
            </code>
        </Form>
    )
});
